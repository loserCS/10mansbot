...
class CacherCog(commands.Cog):
    ...
    def save(self):
        ...
        for guild in self.bot.guilds:
            ...
            guild_data['queue']['banned'] = [user.id for user in guild_queue.banned]
        ...
        with open(self.guild_data_file, 'w+') as f:
            json.dump(data, f, indent=4)  # Dump dict to JSON

    @commands.command(name='save')
    @commands.is_owner()
    async def msave(self, ctx):
        self.save()
        await ctx.send('Saved guild data')
        print(f'{self.timestamp()} Manually saved guild data')
    ...
    def load(self):
        ...
        for guild_id, guild_data in data.items():
            ...
            if guild_queue and 'queue' in guild_data:
                ...
                banned = guild_data['queue']['banned']
                ...
                guild_queue.banned = [self.bot.get_user(id) for id in banned if self.bot.get_user(id)]
